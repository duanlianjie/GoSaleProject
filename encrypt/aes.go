...
//16, 24, 32 位字符串的话，分别对应 AES-128，AES-192，AES-256 加密方法
//key 不能泄露
...
//AesEncrypt 实现加密
func AesEncrypt(originalData []byte, key []byte) ([]byte, error) {
	//创建加密算法实例
...
func AesDeCrypt(encypted []byte, key []byte) ([]byte, error) {
	...
	originalData := make([]byte, len(encypted))
	//这个函数也可以用来解密
	blockMode.CryptBlocks(originalData, encypted)
	//去除填充字符串
	originalData, err = PKCS7UnPadding(originalData)
	if err != nil {
		return nil, err
	}
...
//EncodePassword 加密base64
func EncodePassword(pwd []byte) (string, error) {
